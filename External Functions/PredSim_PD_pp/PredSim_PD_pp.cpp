...
#include <OpenSim/Actuators/PDController.h>
...
constexpr int n_in = 2 + 1; // reference pos reference vel previous gait
constexpr int n_out = 1; // actuator torque
/// number of elements in input/output vectors of function F
constexpr int ndof = 18;        // # degrees of freedom (including locked)
constexpr int NX = ndof * 2 + 2;      // # states plus two for references
constexpr int NU = ndof;        // # controls
constexpr int NR = ndof + 4*3;    // # residual torques + # joint origins + actuator torque
...
int F_generic(const T** arg, T** res) {
   ...
   //ground_pelvis
    ...
    // Read inputs
    std::vector<T> x(arg[0], arg[0] + NX - 2);
	std::vector<T> xref(arg[0] + NX - 1, arg[0] + NX);
    std::vector<T> u(arg[1], arg[1] + NU);

    // States and controls
	T ua[NU]; /// joint accelerations (Qdotdots) - controls
	Vector QsUs(NX - 2); /// joint positions (Qs) and velocities (Us) - states, +2 for reference
	Vector XRef(2); /// joint positions (Qs) and velocities (Us) - states, +2 for reference

    // Assign inputs to model variables
    /// States
	for (int i = 0; i < NX - 2; ++i) QsUs[i] = x[i];
	for (int i = 0; i < 2; ++i) XRef[i] = xref[i];
...
int main() {
	...
	for (int i = 0; i < NX - 2; ++i) x[i] <<= 0;
	for (int i = 0; i < 2; ++i) xref[i] <<= 0;
	for (int i = 0; i < NU; ++i) u[i] <<= 0;

	const Recorder* Recorder_arg[n_in] = { x,u,xref };
